  ...
  export class HistorysComponent {
    ...
    load() {
      this.loading = true
      this.rs.post("/app/history/api/history/search", this.query).subscribe(res => {
    ...
    delete(id: number, size?: number) {
      this.rs.get(`/app/history/api/history/${id}/delete`).subscribe(res => {
        if (!size ) {
    ...
    read(data: any) {
      this.rs.get(`/app/history/api/history/${data.id}/read`).subscribe(res => {
        data.read = true;
